...
from ESC10Customdataset_Train import LAEData_Train
from ESC10Customdataset_Test import LAEData_Test
...
SAVE_DIR = 'models'
MODEL_SAVE_PATH = os.path.join("/media/mani/Ph.D./PHD_Journals/AK/Explainable ASC/ESC-10", 'DualESC10Base_CNN.pt') # Define Path to save the model 
Teacher_Model.load_state_dict(torch.load(MODEL_SAVE_PATH))
#print(Teacher_Model.PAM.weight)
...
#======================================================================Implement GRAD-CAM=================================================================================
#feature_extractor = torch.nn.Sequential(*list(Teacher_Model.children()))
...
#     plt.scatter(X_2d[np.where(test_preds == i), 0], X_2d[np.where(test_preds == i), 1], c=c, label=label)
# plt.legend()
# plt.xticks(size=12)
# plt.yticks(size=12)
...
Teacher_Model.eval()
Filename=np.load('ESC10TrainData.npy',allow_pickle=True)
Class_Label=np.load('ESC10TrainLabel.npy',allow_pickle=True)
key=Filename[0]
print(key)
...
ES_Data=test_transformations(ES_Data)
#print(feature_extractor[0])
ES_Data=ES_Data.unsqueeze(0)
ES_Data=ES_Data.to(device)
Pred=Teacher_Model(ES_Data).argmax(dim=1)
#print(Pred)
params = list(Teacher_Model.parameters())
print(params[58].shape)
weight = np.squeeze(params[-59].data.cpu().numpy())
#print(weight[8])
...
img_variable = Variable(ES_Data)
logit = Teacher_Model(ES_Data)
